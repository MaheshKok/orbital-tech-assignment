...
### Decision: PostgreSQL - When to Use
...
-- Usage snapshots for analytics (optional)
...
### Current Scale (This Task)

- ~110 messages
- ~8 unique report types
...
**Problem:** 10,000+ messages per billing period
...
3. **Database-Backed Storage**
   - Store processed usage data in PostgreSQL
   - Query with pagination and sorting at DB level
   - Add indexes on sortable columns

#### Challenge 2: External API Rate Limits

**Problem:** Fetching reports/:id for each message
...
async def get_reports_batch(report_ids: list[int]) -> dict[int, dict]:
    ...
    return {rid: result for rid, result in zip(unique_ids, results)}
```
...
async def get_report_cached(report_id: int) -> dict:
    # Check Redis first
    cached = await redis.get(f"report:{report_id}")
    if cached:
        return json.loads(cached)

    # Fetch and cache for 1 hour
    report = await fetch_report(report_id)
    await redis.setex(f"report:{report_id}", 3600, json.dumps(report))
    return report
```

3. **Background Sync Job**
   - Periodically sync all reports to local DB
   - Serve from DB, never hit external API in request path

#### Challenge 3: Multi-Tenant Support

**Problem:** Multiple customers with separate billing
...
**Problem:** Show live usage as messages come in
...
async def stream_usage():
    async def event_generator():
        while True:
            new_data = await check_for_updates()
...
**Problem:** Historical usage trends, cost forecasting
...
def aggregate_daily_usage():
    ...
    usage = fetch_usage_for_date(yesterday)
    store_aggregate(yesterday, sum(u.credits_used for u in usage))
```
...
4. □ Cost breakdown tooltip in UI
...
1. **Backend first** - Get the API working, test with curl
2. **Frontend first** - Mock the API, build the UI
3. **Full stack parallel** - Both simultaneously with clear interface

**Recommendation:** Backend first (simpler, validates data flow), then frontend.
