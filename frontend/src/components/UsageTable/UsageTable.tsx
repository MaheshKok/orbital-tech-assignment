...
export function UsageTable({ data }: UsageTableProps) {
  const { sortOrder, getDirection, getSortPriority, toggleSort } = useUrlSortState();
  ...
  useEffect(() => {
    ...
    const hasChanged = currentIds.length !== dataIds.length ||
    ...
    <div className="overflow-hidden rounded-lg border border-gray-200">
      ...
      <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
        <p className="text-sm text-gray-600">
          Showing <span className="font-medium">{sortedData.length}</span> messages
        </p>
      </div>
    </div>
  );
