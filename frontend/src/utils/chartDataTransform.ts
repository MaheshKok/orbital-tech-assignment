...
import { extractUTCDate, formatChartDate, formatChartTooltipDate } from './dateFormatters';
...
export function calculateTotalCredits(usage: UsageItem[]): number {
  return Math.round(usage.reduce((sum, item) => sum + item.credits_used, 0) * 100) / 100;
}
